...
function createWhereAndParams({ whereInput, varName, chainStr, node, context, recursing, }) {
    ...
    function reducer(res, [key, value]) {
        ...
        const { fieldName, isAggregate, operator } = match?.groups;
        const isNot = operator?.startsWith("NOT") ?? false;
        const dbFieldName = (0, map_to_db_property_1.default)(node, fieldName);
        const coalesceValue = [...node.primitiveFields, ...node.temporalFields, ...node.enumFields].find((f) => fieldName === f.fieldName)?.coalesceValue;
